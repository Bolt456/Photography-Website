...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($username == $user['username'] && $phone_no == $user['phone_no'] && $email == $user['email']) {
        ...
        if ($stmt->execute()) {
            ...
            // Reflect changes in form fields immediately
            $user['email'] = $email;
...
<head>
    ...
    <style>
        body {
            background: linear-gradient(135deg,rgb(56, 55, 46),rgb(231, 221, 164));
...
<body>
    ...
    <script>
        ...
        // Update form fields dynamically after success
        <?php if ($message && strpos($message, 'Profile updated successfully') !== false): ?>
            document.querySelector('[name="email"]').value = "<?php echo htmlspecialchars($user['email']); ?>";
            document.querySelector('[name="username"]').value = "<?php echo htmlspecialchars($user['username']); ?>";
            document.querySelector('[name="phone_no"]').value = "<?php echo htmlspecialchars($user['phone_no']); ?>";
        <?php endif; ?>
    </script>
</body>
</html>
